...
```bash
git clone https://github.com/dhnihaoya/Aptgent.git
cd Aptgent
```
...
6. **初筛打分**：按预测概率对候选排序并取 top-k 截断。
7. **特异性筛选**：将候选与靶标类似物交叉比对，去除非特异性结合的候选。
8. **对接参数设置**：LLM 建议对接参数（数值经钳位校验后生效），你确认或修改。
9. **分子对接**：AutoDock Vina 执行对接计算（需要 Vina 已安装）。
10. **空间排序**：基于空间互作矩阵重新排序候选。
11. **最终报告**：导出 Markdown 和 JSON 格式的报告，完成工作流。
